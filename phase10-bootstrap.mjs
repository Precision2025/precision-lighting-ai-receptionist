...
// JOSHUA_CANONICAL_IDENTITY_FUZZY_PLACEHOLDER_V2
...
function officeDisplayItem(item){if(!item||typeof item!=='object')return null;const id=officeJobId(item);if(!id)return null;let customer=officeFirstBrand(item.customer,item.customerName,item.subscriber,item.subscriberName,item.serviceChannelCustomerName,item.serviceChannelSubscriberName,item.client,item.clientName,item.jobSheetCustomer,item.sheetCustomer,item.jobName,item.jobSheetJobName,item.sheetJobName,item.locationName);let location=officeFirstIdentity(item.locationName,item.location,item.serviceChannelLocationName,item.storeName,item.siteName,item.jobName,item.jobSheetJobName,item.sheetJobName,item.clockSharkJobName,item.address);if(!customer&&location&&!/^\d/.test(location))customer=officeBrand(location);if(!customer)customer=officeIsServiceChannel(item)?'ServiceChannel job':'Unknown customer';if(location===customer)location='';if(!officeIdentity(item.customer)&&customer&&!officePlaceholderIdentity(customer))item.customer=customer;if(!officeIdentity(item.customerName)&&customer&&!officePlaceholderIdentity(customer))item.customerName=customer;if(!officeIdentity(item.locationName)&&location)item.locationName=location;return {...item,trackingNumber:id,customer,customerName:customer,locationName:location}}
