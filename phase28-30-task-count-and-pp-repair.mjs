...
/*
 * Joshua Phase 28.30 — task count + ambiguous PP/AA repair + open-task reconciliation
 *
 * Fixes regressions visible after Phase 28.29:
 *   1) completed tasks must not count as open work;
 *   2) bare Job Sheets shorthands PP and AA are not authoritative by themselves;
 *   3) orphaned AUTO-GENERATED primary workflow tasks must not survive forever
 *      after canonical work-order identity/status cleanup.
 *
 * Explicit ServiceChannel evidence and legitimate manual/callback/documentation
...
function isClearlyGeneratedTask(task = {}) {
  const sourceBody = lower([
    task.source,
    task.notes,
    task.closedReason
  ].filter(Boolean).join(" "));
  ...
  return Boolean(
    ...
    task.workflowType ||
    task.actionLabel ||
    /joshua|phase\s*\d+|atomic workflow|workflow authority|reconciler|auto.?generated|automation/.test(sourceBody)
  );
}
...
function canonicalTrackingIndex(workOrders = {}) {
  const index = new Set();
...
function patchServerOpenTaskCount() {
  ...
  // Keep the original Phase 15 anchor intact, then remove completed history.
...
function patchPhase2829OrphanPrimaryHandling() {
  const phasePath = new URL("./phase28-29-atomic-workflow-snapshot.mjs", import.meta.url);
  if (!fs.existsSync(phasePath)) return;

  let source = fs.readFileSync(phasePath, "utf8");
  if (source.includes("JOSHUA_PHASE28_30_ORPHAN_PRIMARY")) return;

  const oldLine = "    if (!workOrder) return task;";
  const replacement = `    if (!workOrder) {
      // JOSHUA_PHASE28_30_ORPHAN_PRIMARY
      // Callback/documentation tasks were already preserved above. Only retire
      // clearly auto-generated primary workflow tasks; manual tasks stay intact.
      const generated = Boolean(
        task.phase2829AtomicPrimaryTask ||
        task.workflowType ||
        task.actionLabel ||
        /joshua|phase\\s*\\d+|atomic workflow|workflow authority|reconciler|auto.?generated|automation/.test(
          lower([task.source, task.notes, task.closedReason].filter(Boolean).join(" "))
        )
      );
      if (primaryClasses.has(klass) && generated) {
        return closeTask(
          task,
          now,
          "Orphaned auto-generated primary workflow task retired; no canonical work order resolves this tracking identity."
        );
      }
      return task;
    }`;
  ...
  if (!source.includes(oldLine)) {
    console.warn("Joshua Phase 28.30: Phase 28.29 orphan-task anchor was not found.");
    return;
  }

  source = source.replace(oldLine, replacement);
  fs.writeFileSync(phasePath, source);
  console.log("Joshua Phase 28.30 hardened orphan primary-task cleanup.");
}
...
function repairPersistedOpenTasks() {
  if (!fs.existsSync(PERSISTENT_FILE)) {
    return { completedNormalized: 0, orphanGeneratedClosed: 0 };
  }
  ...
  try {
    ...
    const canonical = canonicalTrackingIndex(data.workOrders);
    const now = new Date().toISOString();
    let completedNormalized = 0;
    let orphanGeneratedClosed = 0;
    ...
    data.tasks = data.tasks.map(task => {
      ...
      // Completed history is terminal. Keep the record, but never count it open.
      if (status === "completed") {
        completedNormalized += 1;
        return closeTask(
          task,
          now,
          "Phase 28.30 normalized completed task to closed history.",
          { phase2830CompletedStatusNormalized: true }
        );
      }

      const klass = taskClass(task);

      // Manual/callback/documentation/general tasks are never removed here.
      if (["callback", "documentation", "general"].includes(klass)) return task;
      if (!PRIMARY_CLASSES.has(klass)) return task;
      if (!isClearlyGeneratedTask(task)) return task;

      const tracking = text(task.trackingNumber);
      if (tracking && canonical.has(tracking)) return task;

      orphanGeneratedClosed += 1;
      return closeTask(
        task,
        now,
        "Phase 28.30 retired orphaned auto-generated primary workflow task; no canonical work order resolves this tracking identity.",
        { phase2830OrphanGeneratedTaskClosed: true }
      );
    });

    if (completedNormalized || orphanGeneratedClosed) {
      data.phase2830 = {
        ...(data.phase2830 || {}),
        completedStatusesNormalized:
          Number(data.phase2830?.completedStatusesNormalized || 0) + completedNormalized,
        orphanGeneratedTasksClosed:
          Number(data.phase2830?.orphanGeneratedTasksClosed || 0) + orphanGeneratedClosed,
        lastTaskReconcileAt: now
      };
      data.updatedAt = now;
      fs.writeFileSync(PERSISTENT_FILE, JSON.stringify(data, null, 2));
    }

    return { completedNormalized, orphanGeneratedClosed };
  } catch (error) {
    console.warn("Joshua Phase 28.30 persisted task reconciliation warning:", error.message);
    return { completedNormalized: 0, orphanGeneratedClosed: 0 };
  }
}
...
function patchGeneratedPanelAmbiguousCodes() {
  ...
  let proposalSeen = 0;
  html = html.replaceAll(proposalLine, () => {
    proposalSeen += 1;
    if (proposalSeen === 1) {
...
patchPhase2829OrphanPrimaryHandling();

const workflowRepair = repairPersistedAmbiguousStates();
const taskRepair = repairPersistedOpenTasks();

console.log(
  `Joshua Phase 28.30 preflight: repaired ${workflowRepair.repairedPP} false PP work order(s), ` +
  `${workflowRepair.repairedAA} false AA work order(s), closed ${workflowRepair.tasksClosed} false PP/AA task(s); ` +
  `normalized ${taskRepair.completedNormalized} completed task(s), closed ${taskRepair.orphanGeneratedClosed} orphan generated task(s).`
);
