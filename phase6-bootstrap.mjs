...
`;
  if (!server.includes(insertion)) throw new Error("Could not find workOrderNeedsAttention in server.js");
...
`openWorkOrders: workOrders.filter(item => !["ready_to_bill", "completed", "closed", "paid"].includes(item.joshuaStatus)).length,
...
app.post("/api/job-sheets/status-sync", async (request, reply) => {
  ...
  const workOrder = updateControlWorkOrder(trackingNumber, applyJoshuaStatusFields(updates));
  addControlEvent({
...
`;
  if (!server.includes(insertion)) throw new Error("Could not find route insertion point");
...
console.log("Joshua Phase 6 installed: canonical Job Sheets statuses, documentation rules, clean exceptions, and status metrics.");
