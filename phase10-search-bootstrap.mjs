...
if (phase10.includes('await import("./server.js")')) {
  throw new Error(
    "Could not disable Phase 10 server startup before Work Order Search patching."
  );
}
...
const SEARCH_MARKER = "JOSHUA_HOME_WORK_ORDER_SEARCH_V1";
const NOTES_MARKER =
  "JOSHUA_PHASE23_8_3_HOME_WORK_ORDER_TECH_NOTES_V1";
...
(function(){
 ...
 const escapeHtml=value=>safe(value).replace(/[&<>"']/g,function(ch){
  return({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[ch];
 });
 ...
 function technicianOptions(){
  ...
  return '<option value="Unassigned Technician">Office / Unassigned</option>'+
   techs
    .filter(t=>t&&t.name&&t.active!==false&&t.status!=="inactive")
 ...
 function renderSearch(){
  const input=el("homeWorkOrderSearchInput");
  const results=el("homeWorkOrderSearchResults");
  ...
  const found=workOrders()
   .filter(order=>matches(order,term))
   .slice(0,25);

  results.innerHTML=found.length
   ?found.map(function(order){
     ...
     const status=escapeHtml(
      safe(order.joshuaStatus||order.state||"unknown").replaceAll("_"," ")
     );
   ...
   :'<div class="queue-empty">No matching work orders found.</div>';
 }

 function detail(label,value){
  return '<div class="job-detail-item"><div class="muted">'+
   escapeHtml(label)+
   '</div><strong>'+
   escapeHtml(value||"—")+
   '</strong></div>';
 ...
 function openWorkOrder(tracking){
  selectedWorkOrder=workOrders().find(
   o=>safe(o.trackingNumber)===safe(tracking)
  )||null;

  if(!selectedWorkOrder)return;

  refreshTechnicianSelects();
  el("homeWorkOrderTitle").textContent=
   "Work Order #"+safe(selectedWorkOrder.trackingNumber);
  el("homeWorkOrderSubtitle").textContent=
   safe(selectedWorkOrder.customer||selectedWorkOrder.locationName||"");

  el("homeWorkOrderDetails").innerHTML=
   detail(
    "Status",
    safe(selectedWorkOrder.joshuaStatus||selectedWorkOrder.state||"unknown")
     .replaceAll("_"," ")
   )+
   ...
   detail(
    "Technician",
    selectedWorkOrder.technician||selectedWorkOrder.assignedTechnician
   )+
   detail(
    "Check-in",
 ...
 async function startIvr(action){
  ...
  const technician=
   el(checkout?"jobCheckoutTechnician":"jobCheckinTechnician").value||
  ...
  message.textContent=
   "Starting "+(checkout?"check-out":"check-in")+" call…";

  try{
   const response=await api("/api/control/ivr",{
    method:"POST",
   ...
   message.textContent=
    "✅ Call started: "+
    response.callSid+
    ". Joshua will update the work order after ServiceChannel confirms success.";
 ...
 function install(){
  const button=el("homeWorkOrderSearchBtn");
  const input=el("homeWorkOrderSearchInput");
 ...
 document.addEventListener("click",function(e){
  ...
  if(result){
   e.preventDefault();
   openWorkOrder(result.dataset.homeWorkOrder);
   return;
  }
  if(e.target.closest("#closeHomeWorkOrderDialog")){
   e.preventDefault();
   closeDialog();
   return;
  }
  if(e.target.closest("#jobCheckinBtn")){
   e.preventDefault();
   startIvr("checkin");
   return;
  }
  if(e.target.closest("#jobCheckoutBtn")){
   e.preventDefault();
   startIvr("checkout");
  }
 });
 ...
 if(typeof oldRefresh==="function"){
  window.refresh=async function(){
   ...
   install();
   if(
    el("homeWorkOrderSearchInput")&&
    el("homeWorkOrderSearchInput").value.trim()
   ){
    renderSearch();
   }
   return result;
...
/*
 * Technician notes patch for the exact dashboard Work Order dialog.
 * This is intentionally applied after the search dialog exists, whether the
 * dialog came from the block above or was already present in the panel.
 */
if (!panel.includes(NOTES_MARKER)) {
  panel = panel.replace(
    "</style>",
    `
/* ${NOTES_MARKER} */
...
if (
  panel.includes(" function openWorkOrder(tracking){") &&
  !panel.includes(" function renderHomeWorkOrderTechNotes(order){")
) {
  panel = panel.replace(
    " function openWorkOrder(tracking){",
  ...
  if(Array.isArray(raw)){
   return raw.map(safe).map(v=>v.trim()).filter(Boolean).join("\\n\\n");
  }
...
/*
 * The expanded formatting used by this replacement may not match the compact
 * historical line exactly, so add the render call before the action-message
 * reset as a second reliable anchor.
 */
if (
  panel.includes('  el("homeWorkOrderActionMessage").textContent="";') &&
  !panel.includes(
    '  renderHomeWorkOrderTechNotes(selectedWorkOrder);\n  el("homeWorkOrderActionMessage").textContent="";'
  )
) {
...
console.log(
  "Joshua Work Order Search installed with visible ClockShark technician notes."
);
